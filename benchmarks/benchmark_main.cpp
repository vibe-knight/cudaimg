...
double runBenchmark(const std::string& name, Func func, int iterations = 100) {
    ...
    std::cout << std::setw(30) << std::left << name 
              << std::setw(12) << std::right << std::fixed << std::setprecision(3) << avgMs << " ms"
              << std::endl;
...
int main() {
    ...
    for (const auto& size : sizes) {
        ...
        // 像素操作
        runBenchmark("Invert", [&]() {
            PixelOperator::invert(gpuImage, output);
        });
    ...
    std::cout << "Sequential (" << batchSize << " images): " 
              << std::fixed << std::setprecision(2) << sequentialTime << " ms" << std::endl;
    ...
    for (int numStreams : {1, 2, 4, 8}) {
        ...
        std::cout << "Pipeline " << numStreams << " streams: " 
                  << std::fixed << std::setprecision(2) << pipelineTime << " ms"
