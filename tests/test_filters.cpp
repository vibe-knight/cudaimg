...
protected:
    void SetUp() override {
        ...
        cudaMemcpy(testImage.buffer.data(), data.data(), 256, cudaMemcpyHostToDevice);
    }
...
TEST_F(FiltersTest, InvalidKernelSize) {
    GpuImage output;
    EXPECT_THROW(Filters::medianFilter(testImage, output, 2), std::invalid_argument);
    EXPECT_THROW(Filters::medianFilter(testImage, output, 8), std::invalid_argument);
}
...
TEST_F(ArithmeticTest, DimensionMismatch) {
    ...
    EXPECT_THROW(ImageArithmetic::add(image1, small, output), std::invalid_argument);
}
