...
TEST_F(ResizerTest, NearestNeighborInterpolation) {
    ...
    GpuImage resized;
    ImageResizer::resize(gpuInput, resized, 64, 64, InterpolationMode::NearestNeighbor);
    cudaDeviceSynchronize();
...
TEST_F(ResizerTest, InvalidParameters) {
    ...
    // 无效尺寸
    EXPECT_THROW(ImageResizer::resize(gpuInput, output, 0, 32), std::invalid_argument);
    EXPECT_THROW(ImageResizer::resize(gpuInput, output, 32, 0), std::invalid_argument);
    EXPECT_THROW(ImageResizer::resize(gpuInput, output, -1, 32), std::invalid_argument);
    
    // 无效缩放比例
    EXPECT_THROW(ImageResizer::resizeByScale(gpuInput, output, 0.0f, 1.0f), std::invalid_argument);
    EXPECT_THROW(ImageResizer::resizeByScale(gpuInput, output, -1.0f, 1.0f), std::invalid_argument);
    
    // 无效图像
    GpuImage invalid;
    EXPECT_THROW(ImageResizer::resize(invalid, output, 32, 32), std::invalid_argument);
}
