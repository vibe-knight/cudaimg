...
void DeviceBuffer::copyFromHostAsync(const void* hostPtr, size_t copySize, cudaStream_t stream) {
    if (hostPtr == nullptr) {
        throw std::invalid_argument("Host pointer is null");
    }
    if (copySize > size_) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(devicePtr_, hostPtr, copySize, cudaMemcpyHostToDevice, stream));
}

void DeviceBuffer::copyToHostAsync(void* hostPtr, size_t copySize, cudaStream_t stream) const {
    if (hostPtr == nullptr) {
        throw std::invalid_argument("Host pointer is null");
    }
    if (copySize > size_) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(hostPtr, devicePtr_, copySize, cudaMemcpyDeviceToHost, stream));
}
