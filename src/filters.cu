    ...
    for (int c = 0; c < channels; ++c) {
        ...
        for (int ky = -radius; ky <= radius; ++ky) {
            for (int kx = -radius; kx <= radius; ++kx) {
                ...
                // 颜色权重
                float colorDist = (neighborValue - centerValue) * (neighborValue - centerValue);
        ...
        output[(y * width + x) * channels + c] = static_cast<unsigned char>(
            fminf(fmaxf(sum / weightSum, 0.0f), 255.0f));
    }
}
...
__global__ void boxFilterKernel(
    ...
    for (int c = 0; c < channels; ++c) {
        ...
        output[(y * width + x) * channels + c] = static_cast<unsigned char>(sum * invArea);
    }
}
...
__global__ void sharpenKernel(
    ...
    for (int c = 0; c < channels; ++c) {
        ...
        if (x > 0) sum -= strength * input[(y * width + x - 1) * channels + c];
        if (x < width - 1) sum -= strength * input[(y * width + x + 1) * channels + c];
        if (y > 0) sum -= strength * input[((y - 1) * width + x) * channels + c];
        if (y < height - 1) sum -= strength * input[((y + 1) * width + x) * channels + c];
...
__global__ void laplacianKernel(
    ...
    for (int c = 0; c < channels; ++c) {
        ...
        if (x > 0) sum -= input[(y * width + x - 1) * channels + c];
        if (x < width - 1) sum -= input[(y * width + x + 1) * channels + c];
        if (y > 0) sum -= input[((y - 1) * width + x) * channels + c];
        if (y < height - 1) sum -= input[((y + 1) * width + x) * channels + c];
...
__global__ void addKernel(
    const unsigned char* src1, const unsigned char* src2, unsigned char* output,
    int size) {
...
// 加权和 Kernel
__global__ void addWeightedKernel(
    const unsigned char* src1, const unsigned char* src2, unsigned char* output,
    int size, float alpha, float beta, float gamma) {
...
// 标量加法 Kernel
__global__ void addScalarKernel(
    const unsigned char* input, unsigned char* output,
    int size, int value) {
...
// 标量乘法 Kernel
__global__ void multiplyScalarKernel(
    const unsigned char* input, unsigned char* output,
    int size, float scale) {
...
void Filters::bilateralFilter(const GpuImage& input, GpuImage& output,
                              int kernelSize, float sigmaSpace, float sigmaColor,
                              cudaStream_t stream) {
    if (!input.isValid()) {
        throw std::invalid_argument("Invalid input image");
    }
    ...
    bilateralFilterKernel<<<grid, block, 0, stream>>>(
        ...
        kernelSize, sigmaSpace, sigmaColor
    );
    
    CUDA_CHECK(cudaGetLastError());
}

void Filters::boxFilter(const GpuImage& input, GpuImage& output,
                        int kernelSize, cudaStream_t stream) {
    if (!input.isValid()) {
        throw std::invalid_argument("Invalid input image");
    }
...
void Filters::sharpen(const GpuImage& input, GpuImage& output,
...
// ImageArithmetic 实现
void ImageArithmetic::add(const GpuImage& src1, const GpuImage& src2, GpuImage& output,
                          cudaStream_t stream) {
    ...
    addKernel<<<gridSize, blockSize, 0, stream>>>(
        src1.buffer.dataAs<unsigned char>(),
        src2.buffer.dataAs<unsigned char>(),
        output.buffer.dataAs<unsigned char>(),
        size
    );
...
void ImageArithmetic::subtract(const GpuImage& src1, const GpuImage& src2, GpuImage& output,
                               cudaStream_t stream) {
    if (!src1.isValid() || !src2.isValid()) {
        throw std::invalid_argument("Invalid input images");
    }
...
void ImageArithmetic::multiply(const GpuImage& src1, const GpuImage& src2, GpuImage& output,
...
void ImageArithmetic::blend(const GpuImage& src1, const GpuImage& src2, GpuImage& output,
...
void ImageArithmetic::absDiff(const GpuImage& src1, const GpuImage& src2, GpuImage& output,
                              cudaStream_t stream) {
    if (!src1.isValid() || !src2.isValid()) {
        throw std::invalid_argument("Invalid input images");
    }
