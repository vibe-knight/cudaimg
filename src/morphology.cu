...
// 生成结构元素掩码
__device__ bool isInStructuringElement(int dx, int dy, int halfSize, int elementType) {
    switch (elementType) {
        case 0:  // Rectangle
            return true;
        case 1:  // Cross
    ...
    for (int c = 0; c < channels; ++c) {
        ...
        for (int ky = -half; ky <= half; ++ky) {
            for (int kx = -half; kx <= half; ++kx) {
                if (!isInStructuringElement(kx, ky, half, elementType)) continue;
...
__global__ void subtractKernel(
    const unsigned char* a, const unsigned char* b, unsigned char* output,
    int width, int height, int channels) {
...
// 辅助函数
static void launchMorphKernel(
    void (*kernel)(const unsigned char*, unsigned char*, int, int, int, int, int),
    const GpuImage& input, GpuImage& output,
    int kernelSize, StructuringElement element, cudaStream_t stream) {
...
void Morphology::close(const GpuImage& input, GpuImage& output,
                       int kernelSize, StructuringElement element,
                       cudaStream_t stream) {
    GpuImage temp;
...
void Morphology::topHat(const GpuImage& input, GpuImage& output,
                        int kernelSize, StructuringElement element,
                        cudaStream_t stream) {
    GpuImage opened;
