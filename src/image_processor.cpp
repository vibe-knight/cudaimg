...
GpuImage ImageProcessor::loadFromMemory(const unsigned char* data,
...
void ImageProcessor::downloadToBuffer(const GpuImage& image, unsigned char* buffer,
                                       size_t bufferSize) {
    if (buffer == nullptr) {
...
GpuImage ImageProcessor::gaussianBlur(const GpuImage& input, int kernelSize, float sigma) {
    GpuImage output;
...
GpuImage ImageProcessor::convolve(const GpuImage& input, const float* kernel, int kernelSize) {
    GpuImage output;
...
std::array<std::array<int, 256>, 3> ImageProcessor::histogramRGB(const GpuImage& input) {
...
GpuImage ImageProcessor::resize(const GpuImage& input, int newWidth, int newHeight) {
    GpuImage output;
...
GpuImage ImageProcessor::resizeByScale(const GpuImage& input, float scaleX, float scaleY) {
    GpuImage output;
