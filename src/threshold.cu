...
__global__ void thresholdKernel(
    ...
    unsigned char thresh, unsigned char maxVal, int type) {
...
// 自适应阈值 Kernel (均值)
__global__ void adaptiveThresholdMeanKernel(
    const unsigned char* input, unsigned char* output,
    int width, int height, int channels,
    unsigned char maxVal, int type, int blockSize, int C) {
...
__global__ void adaptiveThresholdGaussianKernel(
    const unsigned char* input, unsigned char* output,
    int width, int height, int channels,
    unsigned char maxVal, int type, int blockSize, int C, float sigma) {
...
void Threshold::threshold(const GpuImage& input, GpuImage& output,
    ...
    thresholdKernel<<<grid, block, 0, stream>>>(
        ...
        thresh, maxVal, static_cast<int>(type)
    );
...
void Threshold::adaptiveThreshold(const GpuImage& input, GpuImage& output,
    ...
    if (type != ThresholdType::Binary && type != ThresholdType::BinaryInv) {
        throw std::invalid_argument("Adaptive threshold only supports Binary and BinaryInv types");
    }
    ...
    if (method == AdaptiveMethod::MeanC) {
        ...
        adaptiveThresholdGaussianKernel<<<grid, block, 0, stream>>>(
            ...
            output.buffer.dataAs<unsigned char>(),
            input.width, input.height, input.channels,
            maxVal, typeInt, blockSize, C, sigma
        );
    }
...
unsigned char Threshold::otsuThreshold(const GpuImage& input, cudaStream_t stream) {
    if (!input.isValid()) {
        throw std::invalid_argument("Invalid input image");
    }
