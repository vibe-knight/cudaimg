...
HostImage ImageIO::loadFromFile(const std::string& filepath) {
    ...
    image.data.assign(data, data + static_cast<size_t>(width) * height * channels);
...
HostImage ImageIO::loadFromMemory(const unsigned char* data, size_t size) {
    ...
    if (pixels == nullptr) {
        throw std::runtime_error(std::string("Failed to decode image from memory: ") +
                                 stbi_failure_reason());
    }
    ...
    image.height = height;
    image.channels = channels;
    image.data.assign(pixels, pixels + static_cast<size_t>(width) * height * channels);
...
std::vector<unsigned char> ImageIO::encodeToMemory(const HostImage& image,
    ...
    int ok = 0;
    if (fmt == "png") {
        ok = stbi_write_png_to_func(stbiWriteCallback, &result,
                                     image.width, image.height, image.channels,
                                     image.data.data(), image.width * image.channels);
    } else if (fmt == "jpg" || fmt == "jpeg") {
        ok = stbi_write_jpg_to_func(stbiWriteCallback, &result,
                                     image.width, image.height, image.channels,
                                     image.data.data(), 90);
    } else if (fmt == "bmp") {
        ok = stbi_write_bmp_to_func(stbiWriteCallback, &result,
                                     image.width, image.height, image.channels,
                                     image.data.data());
    } else if (fmt == "tga") {
        ok = stbi_write_tga_to_func(stbiWriteCallback, &result,
                                     image.width, image.height, image.channels,
                                     image.data.data());
    } else {
...
HostImage ImageIO::loadFromFile(const std::string&) {
    throw std::runtime_error("ImageIO requires stb_image. Build with -DGPU_IMAGE_ENABLE_IO=ON");
}

bool ImageIO::saveToFile(const HostImage&, const std::string&) {
    throw std::runtime_error("ImageIO requires stb_image_write. Build with -DGPU_IMAGE_ENABLE_IO=ON");
}
...
std::vector<unsigned char> ImageIO::encodeToMemory(const HostImage&, const std::string&) {
    throw std::runtime_error("ImageIO requires stb_image_write. Build with -DGPU_IMAGE_ENABLE_IO=ON");
}
