...
__global__ void histogramKernelShared(
    const unsigned char* input, int* histogram,
    int width, int height, int channels, int targetChannel) {
...
// 简单直方图 Kernel（用于小图像）
__global__ void histogramKernelSimple(
    const unsigned char* input, int* histogram,
    int width, int height, int channels, int targetChannel) {
...
__global__ void equalizeKernel(
    ...
    const int* cdf, int width, int height, int channels,
    int cdfMin, int totalPixels) {
    ...
    if (x < width && y < height) {
        for (int c = 0; c < channels; ++c) {
            ...
            // 直方图均衡化公式
            float newValue = (float)(cdf[value] - cdfMin) / (totalPixels - cdfMin) * 255.0f;
            output[idx] = static_cast<unsigned char>(min(max(newValue + 0.5f, 0.0f), 255.0f));
        }
    }
}

// HistogramCalculator 实现
std::array<int, HistogramCalculator::NUM_BINS> HistogramCalculator::calculate(
    const GpuImage& input, cudaStream_t stream) {
...
std::array<int, HistogramCalculator::NUM_BINS> HistogramCalculator::calculateChannel(
    const GpuImage& input, int channel, cudaStream_t stream) {
