    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            if (channels >= 3) {
                ...
                image.at(x, y, 2) = static_cast<unsigned char>((x + y) * 128 / (width + height)); // B
                if (channels == 4) {
                    image.at(x, y, 3) = 255;  // A
                }
            } else {
                // 灰度渐变
                image.at(x, y, 0) = static_cast<unsigned char>((x + y) * 255 / (width + height));
            }
        }
    }
...
void printImageStats(const HostImage& image, const std::string& name) {
    ...
    std::cout << name << ": " << image.width << "x" << image.height 
              << "x" << image.channels
