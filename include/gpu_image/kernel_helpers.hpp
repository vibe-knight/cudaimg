...
inline void calcGridBlock2D(int width, int height, dim3& grid, dim3& block) {
    ...
    grid  = dim3(static_cast<unsigned>((width  + kBlockSize2D - 1) / kBlockSize2D),
...
inline void ensureOutputSize(GpuImage& output, int width, int height, int channels) {
...
// 验证输入图像有效，否则抛异常
inline void validateInput(const GpuImage& input, const char* context = "Invalid input image") {
    if (!input.isValid()) {
...
// 验证两张图像尺寸匹配
inline void validateSameSize(const GpuImage& a, const GpuImage& b,
