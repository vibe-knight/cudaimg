...
public:
    ...
    // 将 GPU 图像复制到指定缓冲区
    void downloadToBuffer(const GpuImage& image, unsigned char* buffer, size_t bufferSize);
    ...
    // 卷积操作
    GpuImage gaussianBlur(const GpuImage& input, int kernelSize = 5, float sigma = 1.0f);
