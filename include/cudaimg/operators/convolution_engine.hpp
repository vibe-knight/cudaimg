...
public:
  ...
  static void convolve(const CudaImage& input, CudaImage& output,
  ...
  static void gaussianBlur(const CudaImage& input, CudaImage& output,
                           int kernelSize, float sigma,
                           cudaStream_t stream = nullptr);

  // Sobel 边缘检测
  // 返回梯度幅值图像
  static void sobelEdgeDetection(const CudaImage& input, CudaImage& output,
                                 cudaStream_t stream = nullptr);

  // 可分离卷积（更高效的实现）
  static void separableConvolve(const CudaImage& input, CudaImage& output,
